<!DOCTYPE html>
<html lang="en" >
<head>
  <meta charset="UTF-8">
  <title>Kenai's Store</title>
  <link rel="stylesheet" href="./style.css">

</head>
<body>
<!-- partial:index.partial.html -->
<!DOCTYPE html>
<html lang="es" dir="ltr">
  <head>
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0">
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="main.css"><link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;700;800&display=swap" rel="stylesheet">
  </head>
  <body>
    <div class="main">
      <div class="container a-container" id="a-container">
        <!-- PARTE PHP -->
      <?php
   require_once("conexionclientes.php");
   if (!$conexion)
   die (" Problemas con la conexion a base de datos");
  
  $registro = $conexion->query("SELECT * FROM CLIENTES WHERE EMAIL='$_REQUEST[email_user]' AND PASSWORD ='$_REQUEST[pass_user]'") or
    die($conexion->error );
    
  if ($reg = $registro->fetch_array())
	  {
      $strEmail = $reg['EMAIL'];
    	$strName = $reg['NOMBRE'];
    	$strLast = $reg['APELLIDO'];
      $strPhone = $reg['TELEFONO'];
      $strDate = $reg['FECHANAC'];
      $strHome = $reg['DOMICILIO'];
      $strRFC = $reg['RFC'];
      $strPASS = $reg['PASSWORD'];
	  } 
	else
    echo 'No existe el Correo seleccionado';
    if (!$registro)
    {
    echo "<script>window.location = 'actualizarclientes.html'</script>";
    }
    $conexion->close();
?>
<!-- FIN PARTE PHP -->
        <form class="form" id="request" method="POST" action = "updateclientes.php">
          <h2 class="form_title title">Actualizar informacion</h2>
          <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64," alt=""><img class="form__icon" src="data:image/svg+xml;base64,"><img class="form__icon" src="data:image/svg+xml;base64,"></div>
          <span class="form__span">Usa tu correo para acceder a tu informacion</span>
          <input class="form__input" type="text" placeholder="Correo" name = "email_user"value = "<?php echo $reg['EMAIL'];?>" >
          <input class="form__input" type="text" placeholder="Nombre" name = "name_user"value = "<?php echo $reg['NOMBRE'];?>" >
          <input class="form__input" type="text" placeholder="Apellido" name = "lastname_user"value = "<?php echo $reg['APELLIDO'];?>">
          <input class="form__input" type="text" placeholder="Contraseña"name = "pass_user"value = "<?php echo $reg['PASSWORD'];?>" >
          <input class="form__input" type="text" placeholder="RFC"name = "rfc_user"value = "<?php echo $reg['RFC'];?>" >
          <input class="form__input" type="text" placeholder="Telefono" name = "phone_user"value = "<?php echo $reg['TELEFONO'];?>" >
          <input class="form__input" type="text" placeholder="Domicilio" name ="home_user"value = "<?php echo $reg['DOMICILIO'];?>">
          <input class="form__input" type="text" placeholder="Fecha de nacimiento" name = "dateborn_user"value = "<?php echo $reg['FECHANAC'];?>" >
          
          <input name = "Submit" type = "submit" value ="Actualizar" class="send_btn">
          
        </form>
        
      </div>
      <div class="container b-container" id="b-container">
        <form class="form" id="b-form" method="" action="">
          <h2 class="form_title title">Eliminar usuario</h2>
          <div class="form__icons"><img class="form__icon" src="data:image/svg+xml;base64," alt=""><img class="form__icon" src="data:image/svg+xml;base64,"><img class="form__icon" src="data:image/svg+xml;base64,"></div>
          <span class="form__span">Este proceso es irreversible</span>
          <input class="form__input" type="text" placeholder="Correo">
          <input class="form__input" type="password" placeholder="Contraseña"><a class="form__link">.</a>
          <button class="form__button button submit">Eliminar</button>
        </form>
      </div>
      <div class="switch" id="switch-cnt">
        <div class="switch__circle"></div>
        <div class="switch__circle switch__circle--t"></div>
        <div class="switch__container" id="switch-c1">
          <h2 class="switch__title title">Te vas?</h2>
          <p class="switch__description description">Revisa el estado de toda tu informacion antes de eliminarla</p>
          <button class="switch__button button switch-btn">Eliminar usuario</button>
        </div>
        <div class="switch__container is-hidden" id="switch-c2">
          <h2 class="switch__title title">No te vayas!</h2>
          <p class="switch__description description">Si no quieres eliminar tu usuario, puedes cambiar toda tu informacion</p>
          <button class="switch__button button switch-btn">Editar informacion</button>
        </div>
      </div>
    </div>
    <script src="main.js"></script>
  </body>
</html>
<!-- partial -->
  <script  src="./script.js"></script>

</body>
</html>
